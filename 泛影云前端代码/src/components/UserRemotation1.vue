  ...
  methods: {
    goE(row){
     ...
     getAll() {
      if(this.User.type!=3){
       //发送异步请求
        this.$axios.get(baseURLStr+backendPort+"/remotations"+"/remotation1/"+this.pagination.currentPage+"/"+this.pagination.pageSize+"/"+this.User.phone+"/"+this.User.type).then((res)=>{
          ...
          this.$axios.get(baseURLStr+backendPort+"/remotations/admin1/"+this.pagination.currentPage+"/"+this.pagination.pageSize).then((res)=>{
            this.pagination.pageSize = res.data.data.size;
            this.pagination.currentPage = res.data.data.current;
            this.pagination.total = res.data.data.total;
            this.dataList = res.data.data.records;
       ...
       if(this.pagination.name!=null){
         if(this.pagination.name.length){
           if(this.User.type==3){
             this.$axios({
              method:'get',
              url:baseURLStr+backendPort+"/remotations/admin1/"+this.pagination.currentPage+"/"+this.pagination.pageSize+"/"+this.pagination.name,
             }).then((res)=>{
                if(res.data.flag){
                  this.rs = 0;
                  this.choice = 1;
              ...
              this.$axios({
                method:'get',
                url:baseURLStr+backendPort+"/remotations/remotation1/"+this.pagination.currentPage+"/"+this.pagination.pageSize+"/"+this.pagination.name+"/"+this.User.phone+"/"+this.User.type,
              }).then((res)=>{
                  if(res.data.flag){
                    this.rs = 0;
                    this.choice = 1;
            ...
            this.getAll();
            this.$message.error("请输入要查询会诊专家的单位！");
            this.choice = 0;
          }
        }else{
          this.getAll();
    ...
    adel(row) {
      this.$confirm("请再次确认是否删除","提示",{type:"info"}).then(()=>{
        this.$axios({
        method:'delete',
        url:baseURLStr+backendPort+"/admin/"+row.file+"/"+row.type,//2023年6月4日修改，以file作为检索
        }).then((res)=>{
          if(res.data.flag){
              this.$message.success("删除成功！");
              this.getAll();
