<template>
  <div class="app-container">
    <div class="box">
      <div >
        <el-input placeholder="医生姓名" v-model="pagination.name" style="width: 200px;" ></el-input>
        ...
        <el-button class="dalfBut" @click="back" v-show="rs==0">返回</el-button>
      ...
      <el-table size="small" current-row-key="id" :data="dataList" stripe highlight-current-row>
        ...
        <el-table-column label="操作" align="center">
          <template slot-scope="scope">
            ...
            <el-button type="primary" size="mini" @click="upgrade(scope.row)" >晋升专家</el-button>
...
<script>
  ...
  methods: {
    ...
    getAll() {
      //发送异步请求
      this.$axios.get(baseURLStr+backendPort+"/admin"+"/"+this.pagination.currentPage+"/"+this.pagination.pageSize).then((res)=>{
    ...
    getBy(){
      if(this.pagination.name!=null||this.pagination.sex!=null||this.pagination.unit!=null){
        let a = this.pagination.name.length;
        this.$axios({
        method:'post',
        url:baseURLStr+backendPort+"/admin"+"/"+this.pagination.currentPage+"/"+this.pagination.pageSize,
        data: { 
    ...
    toInfo(row){
      this.$router.push({
        path:'/doctors/info',query:{email:row.phone}
      })
    },
    del(row) {
    ...
    upgrade(row){
      this.$axios.put(baseURLStr+backendPort+"/admin/"+row.phone,{type:2}).then((res)=>{
        if(res.data.flag){
    ...
    handleAdd () {
      this.formData.physician = this.User.phone;
        this.$axios.post(baseURLStr+backendPort+"/admin/doctors",this.formData).then((res)=>{
          //判断当前操作是否成功
          if(res.data.flag){
